...
RSpec.describe Admin::ManageUsersController, type: :controller do
  ...
  describe 'current_app_access filter - Issue #1168' do
    ...
    describe '#filter_params_permitted' do
      it 'returns nil for current_app_access when no filter params are given (no sticky default)' do
        expect(controller.send(:filter_params_permitted)).to be_nil
      end
    end
    ...
    describe 'GET #index with current_app_access filter' do
      it 'returns all users when no filter is set (no default applied)' do
        get :index, params: { filter: { disabled: nil } }
