...
class DefaultSettings
  ...
  DefaultLogo = 'app_logo.jpg'
  DefaultSchemaOwner = 'fphs'
  TwoFactorAuthIssuer = 'FPHS Apps'
  PageTitle = 'FPHS'
  DefaultShortLinkS3Bucket = (Rails.env.production? ? 'fphs.link' : 'test-shortlink.link')
  DefaultShortLinkLogS3Bucket = (Rails.env.production? ? 'url-shortener-logs.fphs' : 'test-restr-url-shortener-logs')
